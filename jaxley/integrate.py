...
def integrate(
    ...
    *,
    ...
    # single vectorized gather. Indexing once per recording emits ~18 HLO
    # instructions each, so the graph (and hence compile time) grew linearly with
    # the number of recordings: 20k recorded neurons produced ~370k instructions
    # and a ~10 min XLA compile, even though the simulation itself runs in ~0.1s.
    # `reset_index` guarantees the index is the recording position (0..n-1).
